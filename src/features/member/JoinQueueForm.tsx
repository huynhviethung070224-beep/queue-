...
export function JoinQueueForm({ disabled = false, onCreate, onFind, onRequestAccess }: JoinQueueFormProps) {
  const [mode, setMode] = useState<'create' | 'find'>('create')
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else setFoundProfile(await onFind(drexelUserId))
  ...
  return <section className="card p-5 sm:p-6" aria-labelledby="member-profile-title">
    ...
    <div className="mb-5 flex gap-2"><Button type="button" variant={mode === 'create' ? 'primary' : 'secondary'} onClick={() => { setMode('create'); setError(''); setFoundProfile(null) }}>Create new profile</Button><Button type="button" variant={mode === 'find' ? 'primary' : 'secondary'} onClick={() => { setMode('find'); setError(''); setFoundProfile(null) }}>Find my profile</Button></div>
    <form onSubmit={(event) => void submit(event)} noValidate className="space-y-5">
      <div><label htmlFor="drexel-user-id" className="form-label">Drexel User ID</label><input id="drexel-user-id" className="form-control" value={drexelUserId} onChange={(event) => setDrexelUserId(event.target.value)} placeholder="For example, vh358" autoCapitalize="none" autoCorrect="off" required /></div>
