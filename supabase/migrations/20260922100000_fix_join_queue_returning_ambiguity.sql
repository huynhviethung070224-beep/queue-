-- Qualify the INSERT ... RETURNING columns because `status` is also an
-- output parameter of this table-returning function.
...
begin
  ...
  if current_queue_entry_id is null then insert into public.queue_entries (session_id, player_id, status, queued_at)
    values (active_session_id, current_player_id, 'waiting', statement_timestamp())
    returning queue_entries.id, queue_entries.status into current_queue_entry_id, current_queue_status; end if;
